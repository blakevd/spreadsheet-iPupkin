...
namespace SS
{
    ...
    public abstract class AbstractSpreadsheet
    {
        ...
        /// This method visits every cell that comes in contact with the 'start' cell, e.g if 
        /// a cell A1 references another cell via a formula it must also look at the cell if references
        /// and it 'visits' each one recursivley but if it finds a reference to the 'start' cell it 
        /// throws a circularExpression error. Then returns a linked list that is in the correct order
        /// of cells that it must go inside and change starting with the 'start' cell
        /// </summary>
